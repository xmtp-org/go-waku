...
func TestStoreQueryMultipleContentFilters(t *testing.T) {
	...
	s := NewWakuStore(nil, nil, MemoryDB(t), 0, 0, utils.Logger())
	var err error
	err = s.storeMessage(protocol.NewEnvelope(msg1, utils.GetUnixEpoch(), defaultPubSubTopic))
	require.NoError(t, err)
	err = s.storeMessage(protocol.NewEnvelope(msg2, utils.GetUnixEpoch(), defaultPubSubTopic))
	require.NoError(t, err)
	err = s.storeMessage(protocol.NewEnvelope(msg3, utils.GetUnixEpoch(), defaultPubSubTopic))
	require.NoError(t, err)
...
func TestStoreQueryForwardPagination(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		err := s.storeMessage(protocol.NewEnvelope(msg, utils.GetUnixEpoch(), pubsubTopic1))
		require.NoError(t, err)
	}
